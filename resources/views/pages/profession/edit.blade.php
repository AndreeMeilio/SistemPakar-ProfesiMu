        ...
        <div class="flex flex-col gap-y-5 max-w-[600px] max-sm:w-full">
            <div class="flex flex-col gap-y-1">
                ...
                @if ($job->id_admin_updated != null)
                    ...
                    <p class="text-grey-secondary mb-1">Oleh : {{ $job->updatedBy->full_name }}</p>
                @endif
            </div>
            <form autocomplete="off" class="flex flex-col gap-y-6" action="{{ route('profesi-digital.update', $job->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
